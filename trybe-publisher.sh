...
Help()
{
   ...
   echo -e "  ${YELLOW}--private${NO_COLOR}            Define o novo repositório como privado (padrão: público)${BR}"
   echo -e "*******************************************************************"
   exit 0
}
...
if [[ $(find . -type f -name trybe-filter-repo.sh | head -n 1) ]] ; then
    ...
    if [[ $(find $SCRIPTS_BASE/repo-filters-by-project/ -type d -name "*$current_project" | head -n 1) ]] ; then
        ...
        echo
        TrybeWarn "Não foi encontrada uma configuração de limpeza para o projeto ${YELLOW}${current_project}!${NO_COLOR}${BR}"
...
git add README.md
git commit -m "README inicial, em construção 🚧" --quiet
